...
public class SigninActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    private void attemptLogin() {
        ...
        if (cancel) {
            ...
            showProgress(true);
            mAuthTask = new postXMLTask(email, password);
            mAuthTask.execute(new String[]{url + "/Ozu/Signin"});
        }
    }
    ...
    private class postXMLTask extends AsyncTask<String, Void, String> {
        ...
        public postXMLTask(String username, String password){
            this.username=username;
            this.password= Base64.encodeToString(password.getBytes(), Base64.NO_WRAP);
        }
            ...
            if(output!=null) {
                try {
                    ...
                    if(status == 0){
                        ...
                        Subscriber subscriber = new Subscriber(model.getInt("UserId"), model.getString("UserName"), model.getString("UserSurname"), model.getString("UserGender"),model.getInt("UserAge"), model.getString("UserMail"));
